...
class Arista():
    ...
    def barrido_aristas(self):
        ...
        while (aux is not None):
            print('Arista: ', aux.info, '- Peso: ', aux.peso)
            aux = aux.sig
...
class Grafo():
    ...
    def profundidad(self, origen):
        ...
        self.barrido_profundidad(origen)
    ...
    def hay_camino(self, origen, destino):
        self.marcar_no_visitado()
        self.existe_paso(origen, destino)
    ...
    def contar_maravillas_tipo(self):
        paises = {}
        aux = self.__inicio
        while aux is not None:
            if aux.datos['pais'] not in paises:
                paises[aux.datos['pais']] = {'Arq': 0, 'Nat': 0}
            if aux.datos['tipo'] == 'Nat':
                paises[aux.datos['pais']]['Nat'] = paises[aux.datos['pais']]['Nat']+1
            else:
                paises[aux.datos['pais']]['Arq'] = paises[aux.datos['pais']]['Arq']+1
            aux = aux.sig
        return paises
